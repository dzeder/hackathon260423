...
describe("ohanafy-memory tool handlers", () => {
  ...
  describe("with an injected fresh store", () => {
    ...
    it("list_decisions filters by scenario and sorts newest-first", async () => {
      await handlers.recordDecision({ scenarioId: "a", note: "first" });
      await new Promise((r) => setTimeout(r, 5));
      await handlers.recordDecision({ scenarioId: "a", note: "second" });
      await handlers.recordDecision({ scenarioId: "b", note: "other" });

      const out = await handlers.listDecisions({ scenarioId: "a" });
    ...
    it("record_decision Zod-rejects empty notes", async () => {
      await expect(
        handlers.recordDecision({ scenarioId: "a", note: "" }),
      ).rejects.toThrow();
    });
  });
