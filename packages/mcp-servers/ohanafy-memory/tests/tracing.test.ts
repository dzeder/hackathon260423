...
const SERVICE_NAME = "ohanafy-plan-mcp-memory";
const TOOL_NAME = "record_decision";

describe("ohanafy-memory tracing.traceTool", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("returns the handler result and emits a tool_complete log with hashed customer id", async () => {
    ...
    const out = await traceTool(TOOL_NAME, { customerId: "c2" }, async () => "ok");
    expect(out).toBe("ok");
...
describe("ohanafy-memory tracing.log base fields", () => {
...
describe("ohanafy-memory tracing.hashCustomer", () => {
