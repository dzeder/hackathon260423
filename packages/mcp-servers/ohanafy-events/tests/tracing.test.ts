...
const TOOL_NAME = "search_events";

describe("ohanafy-events tracing.traceTool", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("returns the handler result and emits a tool_complete log with hashed customer id", async () => {
    ...
    const out = await traceTool(TOOL_NAME, { customerId: "c1" }, async () => 42);
    expect(out).toBe(42);
...
describe("ohanafy-events tracing.log base fields", () => {
...
describe("ohanafy-events tracing.hashCustomer", () => {
